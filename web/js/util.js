...
class Util {
  ...
  ***/
   ...
   *
   * @param cost_matrix {Array} a 2D cumulative cost matrix for DTW
   * @param pos {Array} the current position
  ...
  static compute_dtw_path (cost_matrix, pos) {
    ...
    let ret_val = [indices[index]].concat(Util.compute_dtw_path(cost_matrix, indices[index]))
    return ret_val
  ...
  static dtw(origin_traj, target_traj, window = 3) {
    let accumulated_cost_matrix = Util.dtw_accumulated_cost_matrix(origin_traj, target_traj, window)
    ...
    let coord_path = Util.compute_dtw_path(accumulated_cost_matrix, [x_len-1, y_len-1]).reverse()
